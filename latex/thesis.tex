...
The first diffusion models were discrete-time Gaussian processes \cite{sohldickstein2015} \cite{ho2020}. These were later reformulated as stochastic differential equations (SDEs) \cite{song2021}. In a diffusion model, the probability path interpolating between a simple distribution and the data distribution is created through a forward noising process modeled by an SDE. Inspired by the reversal of the forward noising process \cite{anderson1982}, these SDEs are chosen such that they have marginal probabilities which can parametrize the generator of the diffusion process using the score function, hence the name score matching \cite{song2020} \cite{lipman2024}. Our discussions regarding the framework of flow/score matching mostly follow the conventions used by \cite{lipman2024}, \cite{holderrieth2026}. Specifically, we consider the SDE we want to learn as moving forward in time, unlike the theory of time-reversal used in the older score matching literature, such as \cite{song2021}.
...
A very important example of a martingale is that of Brownian motion.
	\begin{definition}[Brownian motion]
		The standard d-dimensional Brownian motion is a stochastic process $\{B_t\}$ such that
		\begin{itemize}
			\item $B_0 = 0$
			\item for all $k$, and all $0 < t_1 < \ldots < t_k$, the random variables \[B_{t_k} - B_{t_{k-1}} \sim N(0, (t_k - t_{k-1})I)\] are independent, where $I$ is the d-dimensional identity matrix.
			\item $B_t$ is almost surely continuous in $t$
		\end{itemize}
	\end{definition}

We intend to construct the Itô integral, which will be essential in our definition of stochastic differential equations. The exposition of the construction follows \cite{oksendal2013}. We begin by defining the class $\nu$ of functions for which the Itô integral will exist. 
...
We skimmed over some details regarding measurability and convergence here. The interested reader is referred to \cite{karatzas1998}.
...
The limit exists in $L^2$ since the set of integrals $\int_S^T \phi_n(t, \omega)dB_t$ forms a Cauchy sequence in $L^2$ by the Itô isometry. In short, we get the definition below.
...
Now we are ready to define stochastic differential equations. Since the most important integrator (and the only one which we will use) for stochastic integrals is Brownian motion, which is nowhere differentiable, we choose to define stochastic differential equations in terms of integrals rather than derivatives.
...
\begin{definition}[Time-homogeneous Itô diffusion]
	A (time-homogeneous) Itô diffusion is a stochastic process $X_t(\omega) = X(t,\omega):[0,\infty) \times \Omega \to \mathbb{R}^n$ satisfying a stochastic differential equation of the form
	\begin{equation*}
		dX_t = b(X_t)dt + \sigma(X_t)dB_t, \quad t \geq s; X_s = x
	\end{equation*}
where $B_t$ is m-dimensional Brownian motion and $b:\mathbb{R}^n \to \mathbb{R}^n$, $\sigma:\mathbb{R}^n \to \mathbb{R}^{n \times m}$ satisfy
...
\begin{equation*}
	L_{CFM}(\theta) = \mathbb{E}[\| u_t^{\theta}(x) - u_t^{target}(x|z)\|^2]
\end{equation*}

is the same as $L_{FM}(\theta)$, up to a constant, and so minimizing one with gradient descent is equivalent to minimizing the other \cite{lipman2024}, since their gradients coincide. 

Flow models were preceded by (denoising) diffusion models, which, in addition to the vector field, also have a stochastic component. The aim of classic score matching, as well as our martingale matching, is to learn the parametrized drift $b_t^{\theta}$. The diffusion coefficient $\sigma_t$ is chosen by us. Having learned the parametrized drift, we can generate new samples of our data distribution $p_{data}$, using one of many numerical SDE solvers. 
...
\begin{equation*}
	L_{SM} = \mathbb{E}[\|s_t^{\theta}(x) - \nabla \log p_t(x)\|^2]
\end{equation*}

and

\begin{equation*}
	L_{CSM} = \mathbb{E}[\|s_t^{\theta}(x) - \nabla \log p_t(x|z)\|^2].
\end{equation*}

Once again, $L_{SM}$ is intractable, but we are able to show that its gradient coincides with $L_{CSM}$.
...
Another intuitive test function to try is that of polynomials. We found empirically that these work better than the Fourier features, but they have the disadvantage that they are computationally heavier, since the number of test functions of degree $p$ is $d + d^2 + \ldots + d^p$, where $d$ is the dimension of our data. This means that we are restricted to both low-degree polynomials and low-dimensional data.

\begin{definition}[Polynomial test functions]
	Suppose our dataset has dimension $n$, and we want to consider polynomials of degree $d$. Then the set of polynomial test functions is
	\begin{equation*}
	\{f(x) = x^k_ix^k_j | i,j \in \{0,\ldots,n\}, k \in \{0,\ldots, d\}\}.
\end{equation*}
\end{definition}

Another set of test functions is Hermite polynomials. We did not find much empirical improvement of regular polynomial test functions, and hence our analysis focused on comparing random Fourier features with polynomials.
...
The polynomial test functions, in addition to performing quite well empirically, have the additional advantage of having quite a nice mathematical interpretation. Consider the Markov operator defined by 
...
\begin{theorem}
	The minimizer of our loss function $L_{ME}$ satisfies
	\begin{equation}
		\mathcal{L}_{\Delta t}^{\theta} f(x) = \frac{K_{t,\Delta t} f(x) - f(x)}{\Delta t} \quad p_t-a.e.
	\end{equation}
\end{theorem}

\begin{proof}
	We condition $L_{ME}$ on $X_t = x$ to give us
	\begin{equation}
		\mathbb{E}[(f(Y_{t+\Delta t}) - f(x) - \Delta t \mathcal{L}_t^\theta f(x))^2 | X_t = x]
	\end{equation}

	which is minimized by

	\begin{equation}
		\Delta t \mathcal{L}_t^\theta f(x) = \mathbb{E}[f(Y_{t+\Delta t}) | X_t = x] - f(x)
	\end{equation}
\end{proof}
...
so that we simply learn the drift. Hence, to also learn the diffusion, we need higher-order terms. Second degree polynomial terms $f(x) = x_i x_j$ give us
...
However, we found that, in practice, one is not very interested in the distribution of $p_{simple}$, as long as one is able to interpolate between it and $p_{data}$. Hence, Gaussian probability paths suffice in most cases, including ours.

\subsection{Generation}
Having learned our drift $b_t^{\theta}$, we want to use it to generate new samples in $p_{data}$ from our synthetic distribution $p_{simple}$. We can do this by simply solving the stochastic differential equation, and thus simulating its path. In our implementation we use the Euler-Maruyama solver, since it is very simple. Algorithm \ref{alg:euler} gives the pseudocode.
...
We can now generate new samples of our $p_{data}$ distribution, simply by sampling an element of $p_{simple}$, and simulating the SDE using Euler-Maruyama to get from $X_0 \sim p_{simple}$ to $X_1 \sim p_{data}$.
...
To distinguish between the quality of different test functions, we had to train on more difficult data distributions. However, we stuck to $2d$ distributions, since they are easy to visualize, and some problems encountered with higher dimensional data, which are discussed below. Fig. \ref{fig:moonF} shows the flow of data learned using Fourier test functions, compared to the ground-truth probability paths. Fig. \ref{fig:checkF} shows the results for learning a checkerboard data distribution and Fig. \ref{fig:circlesF} the results for a circles distribution.
...
better. Another would be to try to mitigate for weaknesses of different test functions by combining them in various ways, to better represent the whole space of test functions. 
